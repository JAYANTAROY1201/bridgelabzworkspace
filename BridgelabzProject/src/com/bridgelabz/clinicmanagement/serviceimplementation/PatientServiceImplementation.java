	...
	public void searchByMobile()
	{
		...
		for (int i = 0; i < UtilityClinic.patJson.size(); i++) {			
			...
			{   
				...
				System.out.print(++count+"   Name:"+pName+"   ID:"+pId+"   Mobile:"+
						pMob+"  Age:"+jsnobj.get("Age")+"\n\n");	
			}
		}
		if(count==0)
		{
	...
	public void takeAppointment()
	{
		...
		for(int i=0;i<UtilityClinic.docJson.size();i++)
		{
			...
			if(obj.get("Doctor's ID").toString().equals(dID))
			{
				...
				{
					new Appointment(obj.get("Doctor's name").toString(),dID,pName,pId,obj.get("Availibility time").toString());
					...
					if(UtilityClinic.appFile==null)
				      ...
				      Appointment app=new Appointment(obj.get("Doctor's name").toString(),dID,pName,pId,obj.get("Availibility time").toString());
